...
class Table {
 ...
 protected:
  ...
  RecordUID& record_uid(void* rec) {
    assert(rec);
    assert(memory_record_size_ ==
           descriptor_.record_size() + sizeof(RecordUID));
    return *reinterpret_cast<RecordUID*>(reinterpret_cast<uint8_t*>(rec) +
                                         descriptor_.record_size());
  }

  const RecordUID record_uid(const void* rec) const {
    return const_cast<Table*>(this)->record_uid(const_cast<void*>(rec));
  }
