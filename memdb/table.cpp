...
const void* GetRecordFieldPtr(const Descriptor& tbl,
                              const void* rec,
                              FieldId field_id) {
  assert(rec);
  const Field* field = tbl.FindField(field_id);
  return field ? (uint8_t*)rec + field->offset : NULL;
}

inline bool SameField(const Field& field1, const Field& field2) {
  return field1.field_id == field2.field_id && field1.type == field2.type &&
         field1.size == field2.size;
}

static bool CompareFields(size_t count1,
                          const Field* fields1,
                          size_t count2,
                          const Field* fields2) {
  if (count1 != count2)
...
void* Table::Insert(RID rid,
                    size_t field_count,
                    const FieldValue* field_values) {
  void* buf = alloca(descriptor_.record_size());
  NewRecord(buf);

  for (size_t i = 0; i < field_count; ++i) {
    const FieldValue& value = field_values[i];
    if (value.field_index >= descriptor_.field_count())
      return nullptr;

    const Field& info = descriptor_.field(value.field_index);
