...
Descriptor::Descriptor(const Field* fields,
                       size_t field_count,
                       size_t record_size,
                       unsigned version)
...
DBUInt32 Descriptor::GetUInt32(const void* rec, unsigned field) const {
  ...
  const char* ptr = reinterpret_cast<const char*>(rec) + fields_[field].offset;
...
const char* Descriptor::GetString(const void* rec, unsigned field) const {
  ...
  return reinterpret_cast<const char*>(reinterpret_cast<const uint8_t*>(rec) +
...
bool Descriptor::SetString(void* rec,
                           unsigned field_index,
                           const char* text) const {
  assert(field_index >= 0);
  assert(field_index < num_fields_);
  return SetRecordFieldString(rec, fields_[field_index], text);
}

bool Descriptor::SetValue(void* record, const FieldValue& value) const {
  assert(value.field_index >= 0);
  assert(value.field_index < num_fields_);

  const Field* finfo = &fields_[value.field_index];
  ...
  if (finfo->type == DB_STRING) {
    ...
    if (value.size != fields_[value.field_index].size) {
      assert(false);
      return false;
    }
...
size_t Descriptor::PackRecord(const void* rec,
                              void* buffer,
                              size_t size) const {
  ...
  for (size_t i = 0; i < field_count(); ++i) {
    ...
    buf.WriteByte(field.packed_field_id);
...
bool Descriptor::UnpackRecord(void* rec,
                              const void* buffer,
                              size_t size) const {
  ByteMessage buf(const_cast<void*>(buffer), size, size);

  while (!buf.end()) {
    FieldId field_id = buf.ReadByte();
    size_t len = buf.ReadWord();

    const Field* field = FindPackedField(field_id);
    if (!field) {
...
int Descriptor::FindFieldIndex(FieldId field_id) const {
...
const Field* Descriptor::FindField(FieldId field_id) const {
  int field_index = FindFieldIndex(field_id);
  return field_index >= 0 ? &fields_[field_index] : NULL;
}
