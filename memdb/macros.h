...
#define DEFINE_FIELD_EX(pid, name, mem, type, look_tid, look_val, look_dis) \
  {                                                                         \
      ...
      static_cast<uint8_t>(cfg::PackedPropertyId::pid),                     \
      offsetof(_class, mem),                                                \
      SIZEOF_MEMBER(_class, mem),                                           \
      memdb::type,                                                          \
      0,                                                                    \
  },
...
#define DEFINE_FIELD_LOOKUP(pid, name, mem, type, look_tid, look_val, \
                            look_dis)                                 \
...
#define END_FIELD_MAP()                                                \
  ...
  ;                                                                    \
  static const memdb::Descriptor descriptor(fields, std::size(fields), \
                                            sizeof(_class), VERSION);  \
  return descriptor;                                                   \
  }
